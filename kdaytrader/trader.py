...
class Trader:
    ...
    def on_candle(self, code: str, now: datetime, ind: pd.DataFrame | None = None) -> Signal | None:
        ...
        sig = self.strategy.evaluate_with_indicators(code, ind, has_pos, bias=ctx.bias, entry_block=ctx.entry_block, exit_hint=ctx.exit_hint, quant=quant)
        if ctx.notes:
            sig.reasons.extend(ctx.notes)
